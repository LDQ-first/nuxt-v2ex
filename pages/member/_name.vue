...
import { user, members } from '../../api'

export default {
  asyncData ({ app, params, error }) {
    return axios.all([
      axios.get(members(params.name)),
      axios.get(user(params.name))
    ]).then(axios.spread((user, topicList) => {
      return {
        user: user.data,
        topicList: topicList.data
      }
    })).catch(error => console.log('member error', error))
  },
  methods: {
    toWebsite (url) {
      window.open(url)
